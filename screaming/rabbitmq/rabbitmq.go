...
type ScreamMessage func(*amqp.Channel, models.Message) error

func ConsumeMessage(ch *amqp.Channel, queueName string, callback ScreamMessage) error {
	msgs, err := ch.Consume(
		queueName,
		"",
		true,
	...
	log.Println("Screaming service is listening for messages...")
	for msg := range msgs {
		...
		if err != nil {
			log.Printf("Error screaming message: %v", err)
			return err
		}
	}
